...
# export-dhcpServer запустить удаленно быстро не удалось, поэтому путь локальный и запускать скрипт нужно с DHCP-сервера
$path_audit = "d:\audit" 
#$path_audit = "\\srv-ad\d$\audit"
$server_list = @("srv-term", "srv-ad", "srv-bc", "srv-ex")
$DHCP_server_name = "srv-ad.gr.guord.local"
...
<# для удаленного запуска скрипта получим нужные модули
...
#<#
# проверка списока всех пользователей
...
# проверка всех настроек DHCP сервера
...
# изменения групп типа "*admin*" "*Админ*"
...
# проверка изменений автозапуска на серверах in $server_list
forEach ($server in $server_list)
{
  $outfile = "ar-"+$server+"_"+$date+".txt"
  PsExec64.exe -accepteula -nobanner -c -e -f \\$server autorunsc.exe -a * -h -s -t -nobanner -o $outfile
